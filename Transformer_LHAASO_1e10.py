...
class ParT_LHAASO(nn.Module):
    def __init__(self, in_dim=5, param_dim=5, embed_dim=128, num_heads=8, num_layers=4, pair_feat_dim=5, dropout=0.1, max_pool=True):
        super().__init__()
...
def train_and_validate(npz_file,
                       param_keys,
                       use_hitsM=False,
                       epochs=50,
                       batch_size=128,
...
if __name__ == "__main__":
    ...
    out_dir = "/home/zhonghua/Filt_Event/transformer/1e10"
    os.makedirs(out_dir, exist_ok=True)
    model = train_and_validate(npz_file, param_keys, use_hitsM=use_hitsM, epochs=100, batch_size=64, lr=1e-4,
                               device=device, max_hits=256, val_fraction=0.2, out_dir=out_dir, patience=10, seed=42)