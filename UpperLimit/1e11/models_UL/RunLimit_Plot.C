...
void RunLimit_Plot() {
    ...
    c1->SaveAs("./figures/1e11_CLs_scan.pdf");
    ...
    mu->setVal(0); 
    mu->setConstant(true); 
    // 强行给所有 alpha 系统误差参数上锁，防止它们为了迎合 0 数据点而跑到 -5 去
    // 强行给所有 alpha 系统误差参数上锁，防止它们为了迎合 0 数据点而跑到 -5 去
    RooArgSet allVars = w->allVars();   // 【修改 1】：去掉 * 号
    for (auto* arg : allVars) {         // 【修改 2】：去掉 allVars 前面的 * 号
        RooRealVar* var = dynamic_cast<RooRealVar*>(arg);
        if (var && TString(var->GetName()).Contains("alpha_")) {
            // 限制在 -2 到 +2 之间，防止过度外推插值导致 yield 变负
            var->setRange(-2.0, 2.0); 
        }
    }
    ...
    RooFitResult *fitRes = model->fitTo(*data, 
                                        ...
                                        Strategy(1), // 1 比 2 速度快且不容易在极小值处迷失
                                        Offset(kTRUE), // 开启似然函数偏移，拯救极小数值的浮点精度
                                        Hesse(kTRUE));
    ...
    frame2->GetYaxis()->SetRangeUser(-10, 10); 
    ...
    c2->SaveAs("./figures/1e11_bkg_fig_hist.pdf");

    cout << "\n>>> All plots saved." << endl;
    if(mu_bkg) cout << "mu_bkg Value: " << mu_bkg->getVal() << " +/- " << mu_bkg->getError() << endl;
    cout << "Observed Limit: " << upperLimit << endl;
