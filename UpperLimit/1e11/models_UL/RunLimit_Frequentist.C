...
void RunLimit_Frequentist() {
    ...
    // ----------------------------------------------------------
    const int    nToys       = 2000;  // 每个扫描点的 toy 数
    const int    nScanPoints = 30;    // CLs 曲线扫描点数
    const double muMin       = 0.0;
    const double muMax       = 0.03; // ← 根据物理预期调整
    mu->setVal(0);
    model->fitTo(*data);
    ...
    // ----------------------------------------------------------
    // 在创建计算器和统计量后添加：
    profLL.SetPrintLevel(1); // 关掉 Minuit 内部的细节
    ...
    c1->SaveAs("./figures/1e11_CLs_scan_freq.pdf");
