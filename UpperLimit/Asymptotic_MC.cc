...
// g++ -std=c++17 Asymptotic_MC.cc `root-config --cflags --libs` -lRooFit -lRooFitCore -lRooStats -o Asymptotic_MC && ./Asymptotic_MC

int main()
{
    // -------------------------- INPUT FILES --------------------------
    std::string sigPath = "/home/zhonghua/Filt_Event/transformer/1e10_Ponly_optimized_old/val_preds_best_sig.txt";
    std::string bkgPath = "/home/zhonghua/Filt_Event/transformer/1e10_Ponly_optimized_old/val_preds_best_bkg.txt";
    ...
    double Nsig_nominal = 40;
    double Nbkg_nominal = 600;
    double muMax = 1;
    int nPoints = 50;
    ...
    gStyle->SetOptStat(0);
    TFile* fout = new TFile("asymptotic_MC.root", "RECREATE");
    ...
    w.writeToFile("workspace_for_Asymptotic_MC.root");
    fout->Write();
    fout->Close();

    std::cout << "Saved results to asymptotic_MC.root\n";
