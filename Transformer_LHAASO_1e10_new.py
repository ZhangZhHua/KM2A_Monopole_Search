...
def warmup_cosine_schedule(optimizer, warmup_epochs, total_epochs, base_lr):
    def lr_lambda(epoch):
        if epoch < warmup_epochs:
            return (warmup_epochs-epoch) / warmup_epochs
        else:
...
def train_and_validate(npz_file,param_keys,use_hitsM=False,epochs=50,batch_size=128,lr=2e-4,device='cuda',max_hits=1024,val_fraction=0.2,
    ...
    indices = torch.randperm(N).tolist()  # 随机打乱索引
    ...
    # model = ParT_LHAASO_Small(in_dim=in_dim, param_dim=param_dim,dropout=0.4,max_pool=False)
    ...
    scheduler = warmup_cosine_schedule(opt, warmup_epochs=10, total_epochs=epochs, base_lr=lr)
...
def test_exp_data(npz_file, param_keys, model_path="par_transformer_best.pth",
    ...
    # fig_path = os.path.join(out_dir, "exp_probs_distribution_no_muon.png")
    fig_path = os.path.join(out_dir, "exp_probs_distribution_optimized.png")
...
if __name__ == "__main__":
    ...
    # npz_file = "/home/zhonghua/data/Dataset_Filted/1e10_V03/sampled_Gonly_1e10_V03_dataset.npz"
    param_keys = ["label", "R_mean", "Eage", "recE", "rec_theta", "rec_phi"]
    # param_keys = ["label", "recE", "rec_theta", "rec_phi"]
    use_hitsM = False
    device = "cuda:3" if torch.cuda.is_available() else "cpu"
    out_dir = "/home/zhonghua/Filt_Event/transformer/1e10_Ponly_strict"
    # out_dir = "/home/zhonghua/Filt_Event/transformer/1e10_Gonly"
    os.makedirs(out_dir, exist_ok=True)
    # model = train_and_validate(npz_file, param_keys, use_hitsM=use_hitsM, epochs=200, batch_size=64, lr=1e-3,
    #                            device=device, max_hits=256, val_fraction=0.2, out_dir=out_dir, patience=100, seed=42)

    # exp_npz="/home/zhonghua/data/Dataset_Filted/Experiment/2022/1e10_V03_0110_test.npz"  # 没有经过muon cut
    # exp_npz="/home/zhonghua/data/Dataset_Filted/Experiment/2022/1e10_V03_2022_dataset.npz" # 经过muon cut
    # exp_npz="/home/zhonghua/data/Dataset_Filted/Experiment/2022/1e10_V03_2022_dataset_little_muon.npz" # 经过muon cut 还有muon的事例
    # exp_npz="/home/zhonghua/data/Dataset_Filted/Experiment/2022/1e10_V03_2022_dataset_no_muon.npz" # 
    ...
    test_exp_data(exp_npz, param_keys, model_path="par_transformer_best.pth",
                  out_dir=out_dir, use_hitsM=use_hitsM, max_hits=256, device="cuda:3")