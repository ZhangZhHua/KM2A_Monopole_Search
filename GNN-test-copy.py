...
def main(data_file,csv_file,  model_file,version='1e9_V04'):
    ...
    # 只选取需要的列  'R_ue',
    params = [ 'R_mean', 'Eage', 'pincness', 'compactness', 'rho40', 'NpE1', 'NpE2']
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_loss:
            ...
            plt.xlim(0, 1)
            plt.legend()
            plt.savefig(f'/home/zhonghua/Filt_Event/figures/GNN_best_model_{version}_csv-proton.png')
            plt.close()
            epochs_no_improve = 0
            
            np.savez(f"/home/zhonghua/Filt_Event/figures/GNN_Val_hist_{version}_csv-proton.npz", bkg_hist=val_probs[bkg_index], sig_hist=val_probs[sig_index])
        else:
            epochs_no_improve += 1
...
def test_model(data_file,csv_file, model_file, version):
    ...
    if os.path.exists(model_file):
        ...
        plt.xlim(0, 1)
        plt.legend()
        plt.savefig(f'/home/zhonghua/Filt_Event/figures/GNN_test_model_{version}-proton.png')
        plt.close()
        thresholds=[0.8, 0.9, 0.95, 0.99]
        for threshold in thresholds:    
...
if __name__ == "__main__":
    ...
    data_file=f"/data/zhonghua/Dataset_Filted/ForTrain/combined_filted_{version}_dataset-proton-test.npz"
    # model_file = f'/home/zhonghua/Filt_Event/models/GNN_best_model_{version}_csv-gamma.pth'
    csv_file = f"/data/zhonghua/Dataset_Filted/ForTrain/combined_filted_{version}_params-proton-test.csv"
